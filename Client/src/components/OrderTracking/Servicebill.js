...
const SaleBillReport = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${apiKey}/generating_saleBill_report?Company_Code=1&Year_Code=4&doc_no=51`);
      if (!response.ok) {
  ...
  const generatePdf = (data) => {
    ...
    logoImg.onload = () => {
      ...
      pdf.setFontSize(12);
      pdf.setLineWidth(0.3);
      pdf.line(10, 90, 220, 90);
      pdf.setFontSize(14);
      const particulars = [
        ["HSN Code", "Item Name", "Description", "Amount"],
        ["17011490", "Suggar", "Data Added Success", "10000"],
      ];
  ...
  return (
    <div id="pdf-content" className="centered-container" style={{width:"30%"}}>
      <button onClick={fetchData}>Service Bill</button>
    </div>
  );
};
