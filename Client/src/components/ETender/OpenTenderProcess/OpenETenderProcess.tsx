...
const ETenderProcess: React.FC = () => {
    ...
    const fetchTenders = async () => {
        try {
            const response = await axios.get(`${apiKey}/get_all_mill_Opentenders`);
            const data: Tender[] = response.data;
            if (isAdmin === 'Y') {
                const OpenFiltereddata = response.data.filter((tender: any) => tender.Tender_Type === 'O');
                setTenders(OpenFiltereddata);
            } else {
                const filteredData = response.data.filter((tender: any) => tender.MillUserId === parseInt(UserIdNew || '0', 10) && tender.Tender_Type === 'O');
    ...
    const handleViewTender = (tender: Tender) => {
        navigate('/open-tender-details', { state: { tender } });
    };
