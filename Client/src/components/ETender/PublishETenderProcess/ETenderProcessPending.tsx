...
const ETenderProcessPending: React.FC = () => {
    ...
    const handleUpdateCloseETenderBid = async () => {
        try {
            ...
            const response = await axios.put(`${apiKey}/updateCloseetender_e_tender_bid`, updateData);
    ...
    return (
        <>
            ...
            <div>
                <TableContainer component={Paper} sx={{ margin: '10px auto', maxWidth: 500 }}>
                    <Table>
                        <TableHead>
                            <TableRow>
                        ...
                        <TableBody>
                            {[
                                { label: 'Mill Tender ID', value: tender.MillTenderId },
                                { label: 'Mill Code', value: tender.Mill_Code },
                                { label: 'Delivery From', value: tender.Delivery_From },
                                { label: 'Sugar Type', value: tender.Sugar_Type },
                                { label: 'Quantity', value: tender.Quantity },
                                { label: 'Packing', value: tender.Packing },
                                { label: 'Season', value: tender.Season },
                                { label: 'Lifting Date', value: tender.Lifting_Date },
                                { label: 'Last Date of Payment', value: tender.Last_Dateof_Payment },
                                { label: 'Rate Including GST', value: tender.Rate_Including_GST },
                            ].map(({ label, value }) => (
                                <TableRow key={label}>
                ...
                <Box display="flex" justifyContent="flex-start" style={{ float: "right" }} sx={{ mr: 7 }}  >
                    ...
                    <Button variant="contained" color="primary" size="small" onClick={handleUpdateCloseETenderBid} sx={{ ml: 1 }} style={{ height: "5vh" }} >
                        Update & Close ETender
                    </Button>
                </Box>

                <div className={styles.container}>
                    <h2>eTender Biddings</h2>
                    {loading ? (
                        <div>Loading...</div>
                    ) : error ? (
