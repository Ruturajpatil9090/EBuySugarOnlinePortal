...
# API route to get the list of companies
@app.route(API_URL + "/companieslistadmin", methods=['GET'])
def companieslistadmin():
    try:
        # Define the raw SQL query using text()
        sql_query = text("SELECT user_id, company_name,Mill_short_name,accoid,ac_code FROM eBuySugar_UserCreation WHERE accoid IS NOT NULL")
